...
namespace akgr {
  ...
  namespace {
    ...
    void drawAspect(gf::RenderTarget& target, const gf::RenderStates& states, AspectValue& aspect, gf::Vector2f relativePosition, const gf::Color4f& color,
      ...
      {
        float percent = aspect.value.asFloat() / aspect.max.asFloat();
...

      {
        ...
        std::string str = std::to_string(aspect.value.asInt()) + '/' + std::to_string(aspect.max.asInt()) + ' ' + name;
  ...
  void AspectRenderer::render(gf::RenderTarget& target, const gf::RenderStates& states) {
    ...
    drawAspect(target, states, m_state.hero.aspects.vp, { 0.015f, 0.095f }, getAspectColor(Aspect::Vitality),
